...
export class TunnelBroker {
		...
		try {
			...
			const timeout = setTimeout(() => {
				...
				if (stored) {
					...
					try {
						stored.res.status(504).send("Request timeout");
					} catch {}
				}
			}, maxTimeout);
	...
	async handleCompletion(req: Request, res: Response) {
		...
		try {
			...
			this.pendingRequests.delete(id);
			try {
				stored.res.status(500).send("Completion Error");
			} catch {}
	...
	handleClientWebSocketConnection(ws: WebSocket, request: http.IncomingMessage) {
		...
		ws.on("close", (code, reason) => {
			...
			if (this.hostConnection && this.hostConnection.readyState === WS_READY_STATE_OPEN) {
				try {
					this.hostConnection.send(JSON.stringify({ type: "ws-close", id: socket_id, code, reason: reason.toString() }));
				} catch {}
			}
		});
		...
		ws.on("error", (err) => {
			...
			if (this.hostConnection && this.hostConnection.readyState === WS_READY_STATE_OPEN) {
				try {
					this.hostConnection.send(JSON.stringify({ type: "ws-error", id: socket_id, error: String(err) }));
				} catch {}
			}
		});
	}
						...
						if (stored) {
							...
							try {
								...
								for (const [k, v] of Object.entries(responseHeaders)) {
									try {
										(stored.res as any).set(k, v as any);
									} catch {}
								}
	...
	private onWebSocketClose(ws: WebSocket, code: number, reason: string, wasClean: boolean) {
		...
		if (att.type === "host") {
			...
			for (const [id, stored] of this.pendingRequests) {
				try {
					stored.res.status(503).send("Host disconnected");
				} catch {}
				clearTimeout(stored.timeout);
			}
			...
			if (this.hostConnection && this.hostConnection.readyState === WS_READY_STATE_OPEN) {
				try {
					this.hostConnection.send(JSON.stringify({ type: "ws-close", id: att.id, code, reason }));
				} catch {}
			}
		}
	}
	...
	private onWebSocketError(ws: WebSocket, error: unknown) {
		...
		if (att.type === "host") {
			...
			if (this.hostConnection && this.hostConnection.readyState === WS_READY_STATE_OPEN) {
				try {
					this.hostConnection.send(JSON.stringify({ type: "ws-error", id: att.id, error: String(error) }));
				} catch {}
			}
		}
	}
	...
	public cleanup() {
		...
		if (this.hostConnection)
			try {
				this.hostConnection.close(1000, "Shutdown");
			} catch {}
